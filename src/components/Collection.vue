<template>
  <div class="content">
    <table class="table-goods" v-if="collection.length">
      <colgroup>
      ...
      <tr
        ...
        <td class="col03">{{item.address}}</td>
        <td class="col04">￥{{ item.price * item.quantity }}</td>
        <td class="col05">
          <span class="btn-text" @click="drop(index)">取消收藏</span>
        </td>
      </tr>
      </tbody>
    </table>
	  ...
	  methods: {
	    getStore () {
	      ...
	      if (gsStore) {
			...
			this.collection=gsStore.collection || []
	      }
	    },
	    setStore () {
	      let gsStore = {
	    ...
	    setCheckedArray () {
	      this.checkedArray = this.collection.map(() => false)
	    },
	    getTotalPrice () {
	      let balance = this.collection.filter((item, index) => this.checkedArray[index])
